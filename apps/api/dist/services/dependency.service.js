...
exports.dependencyService = {
     ...
     */
    analyzeSymbolImpact: async (projectId, symbolName) => {
        if (!symbolName || !symbolName.trim()) {
            throw api_error_1.ApiError.badRequest('Symbol name is required for impact analysis');
        }
     ...
     */
    refactorDependency: async (projectId, dto) => {
        const report = await exports.dependencyService.getDependencyGraph(projectId);
        const dep = report.dependencies.find((d) => d.id === dto.dependencyId);
        if (!dep) {
            throw api_error_1.ApiError.notFound(`Dependency not found with ID: ${dto.dependencyId}`);
        }
