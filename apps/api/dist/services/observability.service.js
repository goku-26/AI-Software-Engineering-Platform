...
exports.observabilityService = {
    ...
    getObservabilityReport: async (projectId) => {
        ...
        const averageLatencyMs = Math.round(agentRoleStats.reduce((acc, curr) => acc + curr.avgLatencyMs * curr.taskCount, 0) / totalTasksExecuted);
        const report = {
     ...
     */
    getToolCallLogs: async (projectId) => {
        const report = await exports.observabilityService.getObservabilityReport(projectId);
        return report.toolCallLogs;
    },
    /**
     * Runs an automated LLM Evaluation Benchmark audit against active agents and models.
     */
    runEvaluationBenchmark: async (projectId) => {
        const report = await exports.observabilityService.getObservabilityReport(projectId);
